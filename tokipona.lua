...
--- MOD_ID: tokipona
--- MOD_AUTHOR: [nyxbird]
--- MOD_DESCRIPTION: Adds a toki pona translation

----------------------------------------------
------------MOD CODE -------------------------

function SMODS.INIT.tokipona()
...
function Game:set_language()
    -- if not self.LANGUAGES then 
        if not(love.filesystem.read('localization/'..G.SETTINGS.language..'.lua') or G.SETTINGS.language == 'tp') or G.F_ENGLISH_ONLY then
        ...
        self.LANGUAGES = {
            ...
            ['tp'] = {font = 10, label = "󱥬󱥔", key = 'tp', beta = true, button = "󱥄󱥔󱤉󱥬󱤆󱥁", warning = {'󱥬󱥁󱤧󱥐󱤂󱦜','󱥞󱥷󱥔󱤉󱥆󱤡󱥄󱤭󱤉󱥼󱤅󱦜','󱥄󱤭'}},
        ...
        local mod = SMODS.findModByID("tokipona")
		self.FONTS = {
			...
			{file = mod.path.."nasin-nanpa-3.1.0.otf", render_scale = self.TILESIZE*10, TEXT_HEIGHT_SCALE = 0.83, TEXT_OFFSET = {x=10,y=-20}, FONTSCALE = 0.1, squish = 1, DESCSCALE = 1}
		}
	...
	if G.SETTINGS.language == 'tp' then
		local mod = SMODS.findModByID("tokipona")
		-- local font_path = mod.path.."nasin-nanpa-3.1.0.otf"
		local lang_path = mod.path.."tp.lua"
		self.localization = assert(loadstring(love.filesystem.read(lang_path)))()
		init_localization()
	else
